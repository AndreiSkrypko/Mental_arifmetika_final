...
class PaymentSettings(models.Model):
    ...
    price_per_lesson = models.DecimalField(max_digits=8, decimal_places=2, verbose_name='Цена за занятие (руб.)')
    price_per_month = models.DecimalField(max_digits=8, decimal_places=2, verbose_name='Цена за месяц (руб.)')
    is_active = models.BooleanField(default=True, verbose_name='Активно')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='Дата создания')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='Дата обновления')

    def __str__(self):
        return f'Настройки оплаты для {self.class_group.name}'
